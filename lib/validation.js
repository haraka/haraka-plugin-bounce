...
function compareHash(expected, actual) {
  ...
  // pad actual to expected length so timingSafeEqual gets equal-size buffers
  const buf2 = Buffer.concat([Buffer.from(actual)], expected.length)

  if (crypto.timingSafeEqual(buf1, buf2)) return { match: true }

  return {
    match: false,
    msg: expected.length === actual.length ? 'hash does not match' : 'hash length mismatch',
  }
}
